...
describe("Option", () => {
  ...
  describe("Option.getOrThrow", () => {
    ...
    it("should throw an error for None", () => {
      const option = Option.none<number>();
      expect(() => option.getOrThrow(new Error("No value"))).toThrow(
        "No value"
      );
    });
  });
  ...
  describe("Option.orElse", () => {
    it("should return the current Some if defined", () => {
      const option = Option.some(10);
      const alternative = Option.some(20);
  ...
  describe("mapping", () => {
    it("should map over a Some value", () => {
      const option = Option.some(10).map((x) => x * 2);
      expect(option.getOrElse(0)).toBe(20);
    });

    it("should flatMap over a Some value", () => {
      const option = Option.some(10).flatMap((x) => Option.some(x * 2));
      expect(option.getOrElse(0)).toBe(20);
    });

    it("should filter Some value", () => {
      const option = Option.some(10).filter((x) => x > 5);
      expect(option.isDefined()).toBe(true);
      expect(option.getOrElse(0)).toBe(10);

      const filteredOption = Option.some(10).filter((x) => x < 5);
